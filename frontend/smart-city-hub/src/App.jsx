...
export default function App() {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('mobility-telemetry-update', (payload) => {
      ...
      if (payload.stepTelemetry && payload.stepTelemetry.length > 0) {
        ...
        const randomUpdate =
          payload.stepTelemetry[Math.floor(Math.random() * payload.stepTelemetry.length)];
        const matchedRoute = payload.routes?.find((r) =>
          r.buses.some((b) => b.id === randomUpdate.busId)
        );
  ...
  const handleToggleSimulation = async () => {
    ...
    try {
      ...
      setLiveLogs((prev) => [
        {
          id: Date.now(),
          msg: nextState ? '🚀 Simulación Urbana INICIADA' : '⏹️ Simulación Urbana DETENIDA',
  ...
  const handleTriggerCrisis = async (eventCode) => {
    try {
      ...
      let crisisMsg = '';
      if (eventCode === 'MARATON_ALAMEDA')
        crisisMsg = '🚨 ALERTA: Maratón Alameda activa. Tráfico cortado en Santa Lucía.';
      else if (eventCode === 'PARTIDO_ESTADIO')
        crisisMsg =
          '🚨 ALERTA: Partido en el Estadio Nacional. Demanda duplicada en Grecia y Providencia.';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span
              ...
              style={{
                backgroundColor: simulationActive ? 'var(--color-green)' : 'var(--text-muted)'
              }}
            ></span>
            <span
              style={{
                fontSize: '0.85rem',
                fontWeight: 600,
                color: simulationActive ? 'var(--color-green)' : 'var(--text-secondary)'
              }}
            >
      ...
      <main className="main-content">
                ...
                <div>
                  <h4 style={{ fontSize: '1rem', color: '#ff8a9a', marginBottom: '0.25rem' }}>
                    {activeEvent === 'MARATON_ALAMEDA'
                      ? 'CRISIS ACTIVA: Maratón Santiago de Chile'
                      : 'EVENTO MASIVO: Encuentro en Estadio Nacional'}
                  </h4>
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <h3 style={{ fontSize: '1.75rem', margin: '0.25rem 0' }}>
                    {kpis ? `${kpis.avgCommuteTimeMin} min` : '...'}
                  </h3>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                    Trayecto centro-periferia
                  </span>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <h3 style={{ fontSize: '1.75rem', margin: '0.25rem 0' }}>
                    {kpis ? `${kpis.avgSpeed} km/h` : '...'}
                  </h3>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                    Flota Red Bus Metropolitana
                  </span>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                    Buses Hacinados (&gt;90% Cap.)
                  </p>
                  <h3
                    style={{
                      fontSize: '1.75rem',
                      margin: '0.25rem 0',
                      color: kpis?.overcrowdedBuses > 0 ? 'var(--color-red)' : 'var(--text-primary)'
                    }}
                  >
              ...
              >
                ...
                <div>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                    Emisiones de CO2 Est.
                  </p>
                  <h3 style={{ fontSize: '1.75rem', margin: '0.25rem 0' }}>
                    {kpis ? `${kpis.co2EmissionsKg} kg/h` : '...'}
                  </h3>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                    Vehículos detenidos en ralentí
                  </span>
                </div>
              </div>
                ...
                <div>
                  ...
                  <h3
                    style={{
                      ...
                      margin: '0.25rem 0',
                      color:
                        kpis?.greenWaveSyncPct > 80 ? 'var(--color-green)' : 'var(--color-yellow)'
                    }}
                  >
                    {kpis ? `${kpis.greenWaveSyncPct}%` : '...'}
                  </h3>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                    Eficiencia de desfases Alameda
                  </span>
                </div>
              </div>
            </div>
              ...
              >
                <div>
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '0.25rem' }}>
                    Panel Docente (Control de Escenarios)
                  </h3>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                    Inyecte perturbaciones a la grilla en tiempo real.
                  </p>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <button
                    ...
                    style={{
                      ...
                      width: '100%',
                      borderColor:
                        activeEvent === 'NORMAL' ? 'var(--color-cyan)' : 'var(--border-color)'
                    }}
                  >
                  ...
                  >
                    {liveLogs.length === 0 ? (
                      ...
                      liveLogs.map((log) => (
                        <div
                          key={log.id}
                          style={{ borderBottom: '1px solid #111827', paddingBottom: '0.2rem' }}
                        >
        ...
        {activeTab === 'od' && <OdMatrixChart matrixData={odMatrix} />}
      </main>
      ...
      >
        &copy; {new Date().getFullYear()} HUB-Arquímedes. Centro de Control Urbano y Gemelos
        Digitales de Alta Performance.
      </footer>
    </div>
  );
}
