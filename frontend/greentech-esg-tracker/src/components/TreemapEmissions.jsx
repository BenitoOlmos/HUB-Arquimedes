          ...
          {facilityEmissions[0] && (
            ...
            >
              <div>
                ...
                <h4 className="text-xs font-bold font-title mt-1.5 text-white truncate">{facilityEmissions[0].name}</h4>
              </div>
          ...
          <div className="h-full flex flex-col gap-1" style={{ width: `${rightWidth}%` }}>
            ...
            {facilityEmissions[1] && (
              ...
              >
                <div className="flex justify-between items-start">
                  <h4 className="text-xs font-bold font-title text-white truncate max-w-[70%]">{facilityEmissions[1].name}</h4>
                  <span className="text-xxs font-mono text-amber-400">{facilityEmissions[1].country}</span>
                </div>
                <div className="flex justify-between items-end">
                  <span className="text-xxs text-slate-400 font-mono">{((facilityEmissions[1].val / grandTotal) * 100).toFixed(1)}%</span>
                  <span className="text-xs font-bold font-mono text-amber-400">{facilityEmissions[1].val.toFixed(1)} t</span>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-1" style={{ height: `${bottomRightHeight}%` }}>
              ...
              {facilityEmissions[2] && (
                ...
                >
                  <div className="flex justify-between items-start">
                    <h4 className="text-xxs font-bold font-title text-white truncate max-w-[65%]">{facilityEmissions[2].name}</h4>
                    <span className="text-xxs font-mono text-blue-400">{facilityEmissions[2].country}</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-xxs text-slate-400 font-mono">{((facilityEmissions[2].val / grandTotal) * 100).toFixed(1)}%</span>
                    <span className="text-xs font-bold font-mono text-blue-400">{facilityEmissions[2].val.toFixed(1)} t</span>
                  </div>
                </div>
              )}
              ...
              {facilityEmissions[3] && (
                ...
                >
                  <div className="flex justify-between items-start">
                    <h4 className="text-xxs font-bold font-title text-white truncate max-w-[65%]">{facilityEmissions[3].name}</h4>
                    <span className="text-xxs font-mono text-purple-400">{facilityEmissions[3].country}</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-xxs text-slate-400 font-mono">{((facilityEmissions[3].val / grandTotal) * 100).toFixed(1)}%</span>
                    <span className="text-xs font-bold font-mono text-purple-400">{facilityEmissions[3].val.toFixed(1)} t</span>
                  </div>
                </div>
              )}
            </div>
