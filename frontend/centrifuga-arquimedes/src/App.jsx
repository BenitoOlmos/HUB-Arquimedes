...
import { RotateCcw, Activity, HelpCircle, Eye, EyeOff } from 'lucide-react';
...
const fallbackPartsData = {
  volute_casing: {
    ...
    description: 'Carcasa exterior que contiene el líquido y lo dirige a la boquilla de descarga. Su forma espiral actúa como difusor, convirtiendo la energía de velocidad en energía de presión.',
  ...
  shaft_sleeve: {
    ...
    maintenanceLogs: '[{"id":"sl-01","date":"2025-01-12","tech":"Héctor Gómez","desc":"Pulido superficial y rectificación menor. Reensamblado con empaquetaduras nuevas."}]'
  },
  wear_rings: {
    id: 'wear_rings',
    name: 'Wear Rings',
...
const mapMeshIdToPartId = (meshId) => {
  ...
  if (name.includes('wear_ring') || name.includes('anillo_de_desgaste') || name.includes('disque')) {
    return 'wear_rings'; // 'disque d'usure' in French CAD
  }
...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchPartDetails = async () => {
      ...
      try {
        ...
        const data = await response.json();
        // Keep the original 3D node name as id in UI
        ...
        if (fallbackPartsData[mappedId]) {
          ...
          // Dynamic mock if the clicked mesh name is unexpected
          setSelectedPart({
            id: selectedPartId,
  ...
  const handleStatusChange = async (partId, newStatus) => {
    ...
    try {
      ...
      // If we are offline/using fallback, we just update the client-side mock
      if (fallbackPartsData[mappedId]) {
  ...
  const handleAddLog = async (partId, logEntry) => {
    ...
    // Optimistic UI Update for selected part details
    ...
    try {
      ...
      // Fallback update on local mock data
      if (fallbackPartsData[mappedId]) {
        let currentLogs = [];
        try {
