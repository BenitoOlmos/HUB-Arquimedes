...
import { CloudLightning, Flame, AlertOctagon, HelpCircle, ShieldAlert, Sparkles, RefreshCw, XCircle } from 'lucide-react';
...
const PRESETS = [
  {
    ...
    severity: 4,
    description: "Crea oleaje extremo y vientos huracanados. Dificulta el tránsito de la ruta Shanghai - Los Ángeles."
  },
  {
    ...
    severity: 5,
    description: "Cierre temporal del paso por encallamiento. Obliga a desviar buques aproximándose a Europa/Asia vía Cabo."
  },
  {
    ...
    severity: 4,
    description: "Paralización aduanera y de esclusas. Interrumpe el flujo comercial de la costa oeste/este."
  },
  {
  ...
  },
  {
    ...
    region: "Los Angeles",
    severity: 5,
    description: "Operaciones del puerto congeladas. Los buques acumulan demoras (demurrage) por descarga."
  }
];
...
const CrisisConsole = ({ activeCrises, onTriggerEvent, onResolveAll, onTriggerAlert }) => {
  ...
  const handleTriggerPreset = async (preset) => {
    ...
    try {
      ...
      onTriggerAlert(`Crisis Activada: ${preset.name} (${preset.region}). Nivel de impacto: ${preset.severity}/5`);
    } catch (err) {
      console.error(err);
  ...
  const handleCustomSubmit = async (e) => {
    ...
    try {
      ...
      onTriggerAlert(`Crisis Personalizada: ${customType} en ${customRegion} (Severidad ${customSeverity}/5)`);
      setCustomRegion('');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '1.5rem' }}>
      ...
      <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="panel-header">
          <div className="panel-title">
            <ShieldAlert size={18} color="var(--color-held)" /> Consola del Docente e Inyección de Crisis
          </div>
          <button 
        ...
        <h3 style={{ fontSize: '0.85rem', marginBottom: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-secondary)' }}>
          Disrupciones Globales Disponibles (Presets)
        </h3>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', overflowY: 'auto', maxHeight: '420px', paddingRight: '4px' }}>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span className="incoterm-badge" style={{ 
                ...
                <p style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>{preset.description}</p>
                
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '6px' }}>
                  <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>Zona: {preset.region} | Tipo: {preset.type}</span>
                  <button 
      ...
      <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <form onSubmit={handleCustomSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.82rem' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '4px', fontWeight: 'bold' }}>Tipo de Evento:</label>
            <select 
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '4px', fontWeight: 'bold' }}>Región o Canal Afectado:</label>
            <input 
              type="text" 
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '4px', fontWeight: 'bold' }}>Severidad (1 a 5):</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              />
              <span style={{ fontWeight: 'bold', fontSize: '1rem', color: getSeverityColor(customSeverity), minWidth: '20px', textAlign: 'center' }}>
                {customSeverity}
              </span>
            </div>
        ...
        <div style={{ marginTop: '1.5rem', background: 'rgba(255, 255, 255, 0.01)', border: '1px solid var(--border-glass)', borderRadius: '8px', padding: '0.75rem' }}>
          <strong style={{ fontSize: '0.78rem', color: 'var(--color-anchor)', display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
            <AlertOctagon size={12} /> Nota Pedagógica:
          </strong>
          <p style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', lineHeight: '1.3' }}>
            Cuando inyectas un bloqueo en el <strong>Canal de Suez</strong>, la simulación del backend detendrá los barcos cercanos. Los alumnos deberán usar la interfaz de 3D/Telemetría para <strong>Desviar el Buque</strong> hacia la ruta alterna (Cabo de Buena Esperanza), lo que aumentará su costo diario en un 35% pero mantendrá la carga en movimiento para evitar multas por demora.
          </p>
        </div>
      </div>
