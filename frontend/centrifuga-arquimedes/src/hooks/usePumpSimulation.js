...
const fallbackPartsData = {
  ...
  mechanical_seal: {
    ...
    description:
      'Evita fugas del fluido bombeado entre el eje giratorio y la carcasa estacionaria de la bomba. Utiliza caras de sellado primarias de carbón y carburo de silicio.',
  ...
  shaft_sleeve: {
    ...
    description:
      'Cilindro de metal reemplazable colocado sobre el eje en la zona del empaque o sello. Protege el eje contra el desgaste, la corrosión y la erosión.',
  ...
  wear_rings: {
    ...
    description:
      'Anillos reemplazables instalados en la carcasa y/o impulsor para proporcionar una holgura de funcionamiento estrecha. Restringe la recirculación del fluido.',
...
const mapMeshIdToPartId = (meshId) => {
  ...
  if (
    name.includes('wear_ring') ||
    name.includes('anillo_de_desgaste') ||
...
export const usePumpSimulation = () => {
  ...
  useEffect(() => {
    ...
    const fetchPartDetails = async () => {
      ...
      try {
        ...
        console.warn(
          `Could not fetch part ${mappedId} (mesh: ${selectedPartId}) from API. Falling back to local data. Error:`,
          error.message
        );
        if (fallbackPartsData[mappedId]) {
          ...
          setSelectedPart({
            id: selectedPartId,
            name:
              selectedPartId.charAt(0).toUpperCase() + selectedPartId.slice(1).replace(/_/g, ' '),
  ...
  const handleStatusChange = async (partId, newStatus) => {
    ...
    try {
      ...
      console.warn(
        'Could not save status change to backend. Local change preserved.',
        error.message
      );
      if (fallbackPartsData[mappedId]) {
  ...
  const handleAddLog = async (partId, logEntry) => {
    ...
    setSelectedPart((prev) => {
      ...
      try {
        logs =
          typeof prev.maintenanceLogs === 'string'
    ...
    try {
      ...
      console.warn(
        'Could not save maintenance log to backend. Local change preserved.',
        error.message
      );
      if (fallbackPartsData[mappedId]) {
        let currentLogs = [];
        try {
          currentLogs =
            typeof fallbackPartsData[mappedId].maintenanceLogs === 'string'
