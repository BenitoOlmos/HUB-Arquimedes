...
const WorkshopPanel = ({
  ...
  const assignableOTs = activeOTs.filter(ot => 
    ot.budgetApproved && 
    ot.status !== 'Listo' && 
    ...
    <div className="panel-view">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-glass)', paddingBottom: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
            Monitoree el progreso de reparaciones en tiempo real, asigne mecánicos y andenes según especialización para maximizar la productividad.
          </p>
        </div>
      </div>
      ...
      <div className="bay-grid">
        {bays.map(bay => {
          ...
          const activeOT = activeOTs.find(ot => ot.andenId === bay.id && ot.status === 'EnEjecucion');
          ...
          return (
            <div key={bay.id} className={`bay-card ${bayStatus}`}>
              ...
              {/* Bay content */}
              {isOffline ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '16px 0', color: 'var(--status-replace)', textAlign: 'center' }}>
                  <AlertTriangle size={32} />
                  <strong style={{ fontSize: '0.82rem' }}>ELEVADOR BLOQUEADO</strong>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                    Falla hidráulica reportada por el instructor. Ningún vehículo puede ser procesado aquí.
                  </span>
                </div>
              ) : activeOT ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div style={{ background: 'var(--bg-primary)', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-glass)' }}>
                    ...
                    <strong style={{ display: 'block', fontSize: '0.82rem', marginTop: '2px', color: 'var(--text-primary)' }}>
                      {activeOT.vehicleLabel}
                    </strong>
                    <span style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.78rem' }}>
                    <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'rgba(2, 132, 199, 0.1)', color: 'var(--accent-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.72rem' }}>
                    ...
                    <div>
                      ...
                      <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>
                        Esp: {mechanic ? mechanic.specialty : 'N/A'} (Rendimiento: {mechanic ? Math.round(mechanic.speed * 100) : 100}%)
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem' }}>
                      ...
                      <strong style={{ color: 'var(--text-primary)' }}>
                        {activeOT.laborHoursReal.toFixed(1)} / {activeOT.laborHoursTarget.toFixed(1)} Hrs
                      </strong>
                    </div>
                    <div className="progress-bar-container">
                      <div 
                        ...
                        style={{ 
                          ...
                          background: progressPercent >= 100 ? 'var(--status-operational)' : 'var(--accent-blue)' 
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  {progressPercent >= 100 ? (
                    ...
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textAlign: 'center', fontStyle: 'italic', padding: '4px 0' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {/* Idle status details */}
                  <div style={{ background: 'rgba(0,0,0,0.02)', padding: '16px 12px', borderRadius: '8px', border: '1px dashed var(--border-glass)', fontSize: '0.78rem', color: 'var(--text-muted)', textAlign: 'center' }}>
      ...
      <div className="section-card" style={{ marginTop: '12px' }}>
        <h3 className="section-title">
          <UserCheck size={18} color="var(--accent-indigo)" /> Disponibilidad y Costos del Personal Técnico
        </h3>
              ...
              {mechanics.map(mec => {
                ...
                return (
                  <tr key={mec.id} style={{ opacity: isSick ? 0.55 : 1 }}>
                    ...
                    <td>{mec.specialty}</td>
                    <td style={{ fontFamily: 'monospace', fontWeight: 'bold', color: 'var(--accent-blue)' }}>
                      {Math.round(mec.speed * 100)}%
                    </td>
                    <td style={{ fontFamily: 'monospace' }}>${mec.costPerDay.toLocaleString()}/día</td>
                    <td>
                      {isSick ? (
                        <span className="badge-status replace" style={{ fontWeight: 'bold' }}>Licencia Médica (Ausente)</span>
                      ) : assignedBay ? (
                        <span className="badge-status operational" style={{ color: 'var(--accent-indigo)', background: 'rgba(99,102,241,0.08)' }}>
        ...
        <div className="modal-overlay" onClick={() => setSelectedBayId(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '0.75rem', fontWeight: '700', color: 'var(--text-secondary)' }}>Seleccione Orden de Trabajo</label>
              <select 
                value={selectedOtId} 
            ...
            {/* Select Mechanic */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px' }}>
              <label style={{ fontSize: '0.75rem', fontWeight: '700', color: 'var(--text-secondary)' }}>Seleccione Técnico / Mecánico</label>
              <select 
                value={selectedMecId} 
              ...
              >
                ...
                {mechanics.filter(m => !(teacherScenarios.sickMechanic && m.id === 'MEC-2')).map(m => (
