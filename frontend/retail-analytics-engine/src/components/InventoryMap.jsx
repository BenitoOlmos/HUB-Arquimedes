...
export default function InventoryMap({ inventories, products, onTransfer }) {
  ...
  inventories.forEach((inv) => {
    const existing = uniqueStores.find(
      (s) => Math.abs(s.lat - inv.lat) < 0.001 && Math.abs(s.lng - inv.lng) < 0.001
    );
    if (existing) {
  ...
  const handleTransferSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(
        `Transferencia exitosa: ${transferQty} unidades enviadas a ${targetStore.name}`
      );
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
              Visualice stock en tiempo real del SKU seleccionado
            </p>
          </div>
        </div>
        <div>
          ...
          >
            <div>
              <h3 style={{ fontSize: '1.2rem' }}>
                Mapa de Distribución Logística (Zonas Metropolitana)
              </h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                Zonas de calor de demanda Same-Day vs disponibilidad de Dark-Stores
              </p>
            </div>
                ...
                <g className="dispatch-truck">
                  ...
                  <circle r="8" fill="#a5b4fc">
                    <animateMotion
                      dur="3s"
                      repeatCount="1"
                      path={`M ${transitAnimation.fromX} ${transitAnimation.fromY} L ${transitAnimation.toX} ${transitAnimation.toY}`}
                    />
                  </circle>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                1. Nodo de Origen (Bodega Principal)
              </label>
              <select
                className="form-select"
                value={fromType}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                2. Nodo de Destino (Haga Clic en el Mapa)
              </label>
              <select
                className="form-select"
                value={toNodeId}
