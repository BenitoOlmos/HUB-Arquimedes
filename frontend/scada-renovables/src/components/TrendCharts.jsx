...
export default function TrendCharts() {
  ...
  const generateMockHistory = (trendType) => {
    ...
    for (let i = 120; i >= 0; i -= 2) {
      ...
      const label = timestamp.toLocaleDateString('es-ES', { day: '2-digit', month: 'short' }) + ' ' + 
    ...
    const fetchHistory = async () => {
      try {
        if (activeTrend === 'vibration') {
          ...
          if (res.ok) {
            ...
            if (dbData.length > 0) {
              const formatted = dbData.map(d => ({
                time: new Date(d.timestamp).toLocaleDateString() + ' ' + new Date(d.timestamp).getHours() + ':00',
                Vibracion: d.value,
          ...
          const res1 = await fetch('/api/scada/history?tagId=INV-02&parameter=TEMPERATURE&limit=60');
          if (res1.ok) {
  ...
  return (
    <div className="hmi-card" style={{ flex: 1, minHeight: '360px', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Chart Workspace */}
      <div style={{ flex: 1, minHeight: '220px', background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '4px' }}>
        {loading ? (
          <div style={{ display: 'flex', height: '220px', justifyContent: 'center', alignItems: 'center', fontFamily: 'var(--font-mono)' }}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {activeTrend === 'vibration' ? (
                ...
                <LineChart data={chartData} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                  ...
                  <XAxis dataKey="time" tick={{ fill: '#7f8fa4', fontSize: 9 }} stroke="var(--hmi-border)" />
                  <YAxis domain={[0, 8]} tick={{ fill: '#7f8fa4', fontSize: 10 }} stroke="var(--hmi-border)" />
                  <Tooltip contentStyle={{ background: 'var(--hmi-panel)', borderColor: 'var(--hmi-border)', color: 'var(--text-bright)' }} />
                  <Legend wrapperStyle={{ fontSize: 10 }} />
                  <ReferenceLine y={5.0} label={{ value: 'Alerta Critica', fill: 'var(--hmi-fault)', fontSize: 10, position: 'top' }} stroke="var(--hmi-fault)" strokeDasharray="3 3" />
                  <Line name="WTG-01 Vibración (Engranaje)" type="monotone" dataKey="Vibracion" stroke="var(--hmi-fault)" strokeWidth={2.5} dot={false} />
                  <Line name="WTG-02 Vibración (Normal)" type="monotone" dataKey="WTG02_Vib" stroke="var(--hmi-online)" strokeWidth={1.5} dot={false} />
                </LineChart>
                ...
                <LineChart data={chartData} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                  <XAxis dataKey="time" tick={{ fill: '#7f8fa4', fontSize: 9 }} stroke="var(--hmi-border)" />
                  <YAxis yAxisId="left" domain={[30, 100]} tick={{ fill: '#7f8fa4', fontSize: 10 }} stroke="var(--hmi-border)" />
                  <YAxis yAxisId="right" orientation="right" domain={[0, 1000]} tick={{ fill: '#7f8fa4', fontSize: 10 }} stroke="var(--hmi-border)" />
                  <Tooltip contentStyle={{ background: 'var(--hmi-panel)', borderColor: 'var(--hmi-border)', color: 'var(--text-bright)' }} />
                  <Legend wrapperStyle={{ fontSize: 10 }} />
                  <Line yAxisId="left" name="INV-02 Temperatura (°C)" type="monotone" dataKey="Temp_INV02" stroke="var(--hmi-fault)" strokeWidth={2} dot={false} />
                  <Line yAxisId="left" name="INV-01 Temperatura (°C)" type="monotone" dataKey="Temp_INV01" stroke="var(--hmi-online)" strokeWidth={1.5} dot={false} />
                  <Line yAxisId="right" name="Irradiancia Solar (W/m²)" type="monotone" dataKey="Irradiancia" stroke="var(--hmi-cyan)" strokeWidth={1} dot={false} strokeDasharray="5 5" />
                </LineChart>
              ) : (
                // 3. Wind speed vs power curve
                <LineChart data={chartData} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                  <XAxis dataKey="windSpeed" tick={{ fill: '#7f8fa4', fontSize: 9 }} stroke="var(--hmi-border)" />
                  <YAxis domain={[0, 3.0]} tick={{ fill: '#7f8fa4', fontSize: 10 }} stroke="var(--hmi-border)" />
                  <Tooltip contentStyle={{ background: 'var(--hmi-panel)', borderColor: 'var(--hmi-border)', color: 'var(--text-bright)' }} />
                  <Legend wrapperStyle={{ fontSize: 10 }} />
                  <Line name="Potencia Generada (MW)" type="monotone" dataKey="Power" stroke="var(--hmi-cyan)" strokeWidth={2} dot={false} />
                  <Line name="Ángulo Pitch (Pala °)" type="monotone" dataKey="pitch" stroke="var(--hmi-yellow)" strokeWidth={1} dot={false} />
                </LineChart>
              )}
            </ResponsiveContainer>
          </div>
