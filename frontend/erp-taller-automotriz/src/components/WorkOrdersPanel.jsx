...
import { ClipboardCheck, CreditCard, Clock, Truck, ShieldAlert, BadgeHelp, Play, DollarSign, Sparkles } from 'lucide-react';
...
const WorkOrdersPanel = ({
  ...
  const activePart = associatedOtId 
    ? activeOTs.find(o => o.id === associatedOtId)?.partsRequired.find(p => p.sku === purchasePartSku)
    : null;
    ...
    <div className="panel-view">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-glass)', paddingBottom: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
            Revise solicitudes de clientes, cotice reparaciones, apruebe presupuestos y despache vehículos listos para facturación.
          </p>
        </div>
      </div>

      {/* Filter Tabs */}
      <div style={{ display: 'flex', gap: '6px', background: 'rgba(15,23,42,0.04)', padding: '4px', borderRadius: '10px', width: 'max-content' }}>
        <button 
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {filteredOTs.length === 0 ? (
          <div style={{ background: 'var(--bg-glass)', border: '1px solid var(--border-glass)', borderRadius: '12px', padding: '32px', textAlign: 'center', color: 'var(--text-muted)' }}>
          ...
          filteredOTs.map(ot => {
            ...
            return (
              <div 
                ...
                style={{ 
                  background: isVip ? 'linear-gradient(135deg, rgba(139, 92, 246, 0.03) 0%, rgba(255, 255, 255, 0.95) 100%)' : 'var(--bg-glass)',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '8px' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <strong style={{ fontSize: '1.05rem', color: 'var(--text-primary)' }}>{ot.id}</strong>
                      {isVip && (
                        <span style={{ background: 'var(--accent-purple)', color: '#ffffff', fontSize: '0.62rem', fontWeight: 'bold', padding: '2px 6px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '2px' }}>
                          <Sparkles size={8} /> VIP URGENTE
                        </span>
                      )}
                      <span className={`badge-status ${statusBadgeClass}`}>{statusText}</span>
                    </div>
                    <strong style={{ display: 'block', fontSize: '0.88rem', marginTop: '6px', color: 'var(--text-secondary)' }}>
                      Vehículo: {ot.vehicleLabel} | Cliente: {ot.clientName}
                    </strong>
                  </div>

                  <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '2px' }}>
                    <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 'bold' }}>Presupuesto Estimado</span>
                    <strong style={{ fontSize: '1.25rem', color: 'var(--accent-blue)', fontFamily: 'monospace' }}>
                ...
                <div style={{ borderTop: '1px solid var(--border-glass)', paddingTop: '10px', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                  <strong>Motivo de Ingreso:</strong> {ot.description}
                  {ot.notes && (
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '4px', background: 'rgba(0,0,0,0.02)', padding: '6px 10px', borderRadius: '6px', borderLeft: '3px solid var(--border-glass)' }}>
                ...
                {ot.partsRequired.length > 0 && (
                  <div style={{ background: 'rgba(0,0,0,0.01)', border: '1px solid var(--border-glass)', borderRadius: '10px', padding: '12px' }}>
                    <span style={{ display: 'block', fontSize: '0.72rem', fontWeight: 'bold', color: 'var(--text-secondary)', marginBottom: '8px', textTransform: 'uppercase' }}>
                        ...
                        return (
                          ...
                          >
                            <span>
                              ...
                              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontFamily: 'monospace', marginLeft: '6px' }}>
                                SKU: {part.sku}
                              </span>
                            </span>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--border-glass)', paddingTop: '12px', gap: '10px' }}>
                  ...
                  {ot.status === 'Listo' && (
                    <button 
                      ...
                      style={{ padding: '6px 16px', gap: '4px', background: 'var(--status-operational)', borderColor: 'var(--status-operational)' }}
                    >
                  ...
                  {ot.status === 'EsperandoRepuestos' && (
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Truck size={14} /> Esperando que lleguen las compras de bodega para habilitar andén de trabajo.
                    </div>
                  )}
                  {ot.status === 'EnEjecucion' && (
                    <div style={{ fontSize: '0.72rem', color: 'var(--accent-blue)', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 'bold' }}>
                      <Clock size={14} /> Vehículo en reparación en el andén de trabajo. Avance el turno para completar.
                    </div>
                  )}
                </div>
      ...
      {purchasePartSku && activePart && (
        <div className="modal-overlay" onClick={() => { setPurchasePartSku(null); setAssociatedOtId(null); }}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>REPUESTO NECESARIO:</span>
            ...
            <div style={{ background: 'var(--bg-primary)', padding: '12px', borderRadius: '10px', border: '1px solid var(--border-glass)', display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '0.8rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <strong style={{ fontFamily: 'monospace' }}>
                  ${Math.round(activePart.cost * suppliers.find(s=>s.id===selectedSupplierId).costMultiplier).toLocaleString()}
                </strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-glass)', paddingBottom: '4px' }}>
                ...
                <strong style={{ fontFamily: 'monospace', color: 'var(--status-replace)' }}>
                  ${(Math.round(activePart.cost * suppliers.find(s=>s.id===selectedSupplierId).costMultiplier) * purchaseQty).toLocaleString()}
                </strong>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '4px' }}>
              <button className="premium-btn" onClick={() => { setPurchasePartSku(null); setAssociatedOtId(null); }}>
                Cancelar
              </button>
              <button 
                className="premium-btn active"
