...
// Seed Pools
const firstNamesM = ["Juan", "Andrés", "Carlos", "Pedro", "Diego", "Luis", "Javier", "Manuel", "José", "Alejandro", "Felipe", "Sebastián", "Rodrigo", "Cristian", "Gabriel", "Ignacio"];
const firstNamesF = ["María", "Sofía", "Laura", "Ana", "Camila", "Valentina", "Francisca", "Isabella", "Carolina", "Gabriela", "Javiera", "Antonia", "Daniela", "Constanza", "Beatriz", "Elena"];
const lastNames = ["González", "Muñoz", "Rojas", "Díaz", "Pérez", "Soto", "Contreras", "Silva", "Martínez", "Flores", "Morales", "Rodríguez", "Valenzuela", "Araya", "Tapia", "Vergara", "Carrasco", "Sandoval", "Fuentes", "Gómez"];
const bloodTypes = ["O+", "A+", "B+", "AB+", "O-", "A-", "B-", "AB-"];
const comorbiditiesPool = ["Hipertensión Arterial", "Diabetes Mellitus Tipo 2", "Asma Bronquial", "EPOC", "Insuficiencia Renal Crónica", "Hipotiroidismo", "Ninguna"];
const allergiesPool = ["Penicilina", "Aspirina", "Sulfas", "Látex", "AINEs", "Cloxacilina", "Ninguna"];
...
const skuTemplates = [
  { name: "Paracetamol 500mg comprimidos", category: "Medicamentos Urgencia", baseCost: 15, provider: "FarmaSalud Distribuidores" },
  { name: "Salbutamol Inhalador 100mcg/dosis", category: "Medicamentos Urgencia", baseCost: 120, provider: "RespiCare S.A." },
  { name: "Epinefrina Ampolla 1mg/ml", category: "Medicamentos Urgencia", baseCost: 800, provider: "Laboratorio Bios" },
  { name: "Atropina Ampolla 1mg/ml", category: "Medicamentos Urgencia", baseCost: 650, provider: "Laboratorio Bios" },
  { name: "Ceftriaxona Inyectable 1g", category: "Medicamentos Urgencia", baseCost: 1500, provider: "FarmaSalud Distribuidores" },
  { name: "Morfina Ampolla 10mg/ml", category: "Medicamentos Urgencia", baseCost: 3200, provider: "FarmaControl Central" },
  { name: "Suero Fisiológico 0.9% 500ml", category: "Medicamentos Urgencia", baseCost: 250, provider: "Gases y Fluidos Clínicos" },
  { name: "Ringer Lactato 500ml", category: "Medicamentos Urgencia", baseCost: 300, provider: "Gases y Fluidos Clínicos" },
  { name: "Kit de Intubación Endotraqueal", category: "Insumos Quirúrgicos", baseCost: 8500, provider: "MedTech Proveedores" },
  { name: "Catéter Intravenoso 18G", category: "Insumos Quirúrgicos", baseCost: 450, provider: "MedTech Proveedores" },
  { name: "Guantes de Nitrilo Estériles (caja 100)", category: "EPP", baseCost: 1200, provider: "Suministros Médicos Chile" },
  { name: "Mascarilla N95 certificada", category: "EPP", baseCost: 1500, provider: "Suministros Médicos Chile" },
  { name: "Pechera impermeable desechable", category: "EPP", baseCost: 350, provider: "Suministros Médicos Chile" },
  { name: "Oxígeno Botella Portátil 10L", category: "Gases Medicinales", baseCost: 18000, provider: "Indura Hospitalaria" }
];
...
export const initializeSimulationData = () => {
  ...
  skuTemplates.forEach(template => {
    ...
    const chosenVars = template.category === "Medicamentos Urgencia" 
    ...
    chosenVars.forEach(v => {
      ...
      for (let pIdx = 1; pIdx <= 10; pIdx++) {
        ...
        pharmacyDb.push({
          ...
          isCritical: template.name.includes("Oxígeno") || template.name.includes("Kit") || template.name.includes("Epinefrina") || template.name.includes("Guantes")
        });
      }
    });
  });
  ...
  console.log(`HIS-Triage: Seeded ${patientsDb.length} patients, ${historyDb.length} history logs, and ${pharmacyDb.length} SKUs in ${Math.round(endTime - startTime)}ms.`);
};
...
const calculateAnalytics = () => {
  ...
  const monthlyAdmissions = Array(12).fill(0).map((_, i) => ({ monthName: ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"][i], admissions: 0, deaths: 0 }));
  const categoryStats = {};
  ...
  historyDb.forEach(log => {
    ...
    categoryStats[log.category].waitTimeSum = (categoryStats[log.category].waitTimeSum || 0) + log.waitTime;
  });
