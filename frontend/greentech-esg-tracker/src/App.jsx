...
import { Leaf, Award, FileText, AlertTriangle, ShieldCheck, RefreshCw, Layers, ShieldAlert, Cpu } from 'lucide-react';
...
const App = () => {
  ...
  const handleReset = async () => {
    try {
      ...
      const data = await response.json();
      if (data.success) {
        showFlashNotification('Simulación reiniciada. Base de datos re-hidratada con ruidos originales.', 'success');
        fetchData();
      }
    } catch (error) {
      console.error(error);
  ...
  const chartData = [
    ...
    { name: 'Actual Netas', emisiones: metrics.netEmissions, fill: metrics.netEmissions <= metrics.targetEmissions ? '#10b981' : '#f59e0b' }
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#070a12] text-slate-100 pb-12">
      ...
      <header className="w-full bg-[#0c1222]/85 border-b border-slate-800/80 px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-40 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xxs font-mono text-slate-400 mt-0.5">SISTEMA CORPORATIVO DE AUDITORÍA Y COMPENSACIÓN DE EMISIONES</p>
          </div>
        </div>
      ...
      {notification && (
        <div className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-lg border shadow-xl flex items-center gap-2 font-mono text-xs max-w-sm transition-all duration-300 animate-slide-in ${
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 mt-6 flex-grow grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          <div className="glass-panel flex flex-col gap-4">
            <h3 className="text-xs font-mono text-cyan-400 tracking-wider">RESUMEN CO2e CORPORATIVO</h3>
            ...
            <div className="flex flex-col gap-2">
              <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800/40 flex justify-between items-center">
                ...
                <span className="text-sm font-bold font-mono text-white">{metrics.totalEmissions?.toFixed(1)} t</span>
              </div>
              <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800/40 flex justify-between items-center">
                <span className="text-xxs font-mono text-slate-400">Compensadas:</span>
                <span className="text-sm font-bold font-mono text-emerald-400">-{metrics.offsetEmissions?.toFixed(1)} t</span>
              </div>
              <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800/40 flex justify-between items-center">
                <span className="text-xxs font-mono text-slate-400">Emisiones Netas:</span>
                <span className="text-sm font-bold font-mono text-cyan-400">{metrics.netEmissions?.toFixed(1)} t</span>
              </div>
            </div>
            ...
            <div className="border-t border-slate-800 pt-3 flex flex-col gap-3">
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm font-bold font-mono ${metrics.reputationScore >= 80 ? 'text-emerald-400' : metrics.reputationScore >= 50 ? 'text-amber-400' : 'text-red-500'}`}>
              ...
              <div className="w-full bg-slate-900 rounded-full h-2 overflow-hidden border border-slate-800">
                <div 
                  className={`h-full transition-all duration-500 ${
                    metrics.reputationScore >= 80 ? 'bg-emerald-500' : 
                    metrics.reputationScore >= 50 ? 'bg-amber-500' : 'bg-red-500'
                  }`}
          ...
          <div className="glass-panel flex flex-col gap-4">
            <h3 className="text-xs font-mono text-cyan-400 tracking-wider">MARCO REGULADOR & RIESGO LEGAL</h3>
            ...
            {metrics.auditErrorsCount > 0 ? (
              <div className="p-3 bg-red-950/20 border border-red-500/30 rounded-lg flex items-start gap-2.5">
                ...
                <div>
                  <h5 className="text-xxs font-bold text-red-400 font-mono">INCUMPLIMIENTO TCFD / LEY MARCO</h5>
                  <p className="text-xxs text-slate-300 font-mono mt-1">
                    Detectadas {metrics.auditErrorsCount} inconsistencias en auditoría de facturas. Multa virtual acumulada:
                  </p>
              ...
              <div className="p-3 bg-emerald-950/20 border border-emerald-500/30 rounded-lg flex items-start gap-2.5">
                ...
                <div>
                  <h5 className="text-xxs font-bold text-emerald-400 font-mono">CONFORME A LA NORMATIVA</h5>
                  <p className="text-xxs text-slate-300 font-mono mt-1">
                    Cero discrepancias detectadas. Inventario 100% calibrado ante entidades reguladoras. Sin multas.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="glass-panel flex flex-col gap-4">
            ...
            <p className="text-xxs text-slate-400 font-mono">
              Reemplaza el embalaje de plástico virgen por cartón reciclado en el diseño del producto para reducir la huella en la cadena de valor (Alcance 3).
            </p>
          ...
          {activeTab === 'dashboard' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="glass-panel flex flex-col h-[380px] md:col-span-1">
                <h3 className="text-xs font-mono text-cyan-400 tracking-wider mb-4">COMPORTAMIENTO ANUAL VS META SBTi</h3>
                <div className="flex-grow w-full">
                  <ResponsiveContainer width="100%" height="95%">
                    <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip 
                        contentStyle={{ background: '#0c1222', border: '1px solid rgba(56, 189, 248, 0.2)', fontSize: '10px', fontFamily: 'JetBrains Mono' }}
                        cursor={{ fill: 'rgba(255,255,255,0.02)' }}
                      />
                      <ReferenceLine y={metrics.targetEmissions} stroke="#ef4444" strokeDasharray="3 3" label={{ value: 'Límite SBTi', fill: '#ef4444', fontSize: 8, position: 'top' }} />
      ...
      <footer className="max-w-7xl mx-auto px-4 md:px-6 w-full mt-6">
        <div className="bg-[#0c1222]/80 border border-slate-800/60 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-xs font-bold font-title text-white">CONSOLA DEL DOCENTE (SIMULACIÓN DE CRISIS)</h4>
              <p className="text-xxs text-slate-400 font-mono mt-0.5">Controla y altera las dinámicas del mercado de bonos y verificación GEI en vivo.</p>
            </div>
          </div>
