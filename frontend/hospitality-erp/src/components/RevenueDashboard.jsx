...
import { ShieldAlert, DollarSign, Percent, TrendingUp, Sliders, Play, Plus, Trash } from 'lucide-react';

const RevenueDashboard = ({ kpis, pickupCurve, onUpdateRules, initialRules, onUpdateOverbook, initialOverbook }) => {
  ...
  useEffect(() => {
    ...
    const fetchRisk = async () => {
      try {
        const res = await fetch(`/api/revenue/overbook/risk?currentOccupancy=${kpis?.occupancyRate || 75}`);
        if (res.ok) {
          ...
          <div className="w-full h-64 font-mono text-xxs">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={pickupCurve} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#090d16', border: '1px solid #334155' }} />
                <Legend wrapperStyle={{ fontSize: '10px' }} />
                <Bar name="Cantidad Reservas" dataKey="count" fill="#06b6d4" radius={[4, 4, 0, 0]} />
                <Bar name="Ingresos Totales ($)" dataKey="revenue" fill="#10b981" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg_col-span-4 bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xxs font-mono text-slate-400 mb-4 leading-relaxed">
              Define el porcentaje de habitaciones a sobre-vender. Una sobreventa óptima minimiza pérdidas por No-Shows (15% hist), pero arriesga penalidades (Walk-outs).
            </p>
            ...
            <div className="space-y-4 font-mono text-xxs">
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="font-bold text-amber-500">{overbookLimit}% (+{overbookRiskData?.overbookedRooms} hab)</span>
                </div>
                <input
                  type="range"
                  min="0"
              ...
              <div className="p-3 bg-slate-950 rounded border border-slate-850 space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-bold px-2 py-0.5 rounded text-xxxxs ${
                    overbookRiskData?.status === 'HIGH_RISK' ? 'bg-red-950 border border-red-900 text-red-300' :
                    overbookRiskData?.status === 'MODERATE_RISK' ? 'bg-amber-950 border border-amber-900 text-amber-300' :
          ...
          {overbookRiskData?.status === 'HIGH_RISK' && (
            <div className="mt-4 p-2 bg-red-950/40 border border-red-900 text-red-350 rounded font-mono text-xxxxs flex items-center space-x-2">
              ...
              <span>Advertencia: La tasa de sobreventa excede la probabilidad de No-Shows. Riesgo inminente de Walk-outs y caída de Reviews.</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 font-mono">
        ...
        <div className="grid grid-cols-1 lg_grid-cols-12 gap-6">
          ...
          <div className="lg_col-span-4 bg-slate-950 border border-slate-850 p-4 rounded-lg">
            <span className="text-xxs font-bold text-slate-350 block mb-3 uppercase">Nueva Regla de Precios</span>
          ...
          <div className="lg_col-span-8 space-y-2">
            <span className="text-xxs font-bold text-slate-350 block mb-2 uppercase">Reglas Activas en el Motor</span>
            <div className="space-y-2">
              {rules.map((rule) => (
                <div key={rule.id} className="bg-slate-950 border border-slate-850 p-3 rounded-lg flex justify-between items-center text-xxs">
                  <div className="space-y-1">
                    <span className="text-slate-100 font-bold block">
                      SI Ocupación &gt; {rule.occupancyThreshold}% y Lead Time &lt; {rule.leadTimeDays} días
                    </span>
              ...
              {rules.length === 0 && (
                <div className="text-center py-8 text-slate-600 italic text-xxs">
                  No hay reglas dinámicas de Yield configuradas. Las tarifas del hotel se mantendrán fijas.
                </div>
              )}
            </div>
          </div>
