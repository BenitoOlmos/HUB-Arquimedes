    ...
    if (parameter === 'BREAKER') {
      ...
      if (scadaLiveState[tagId]) {
        if (!value) {
          ...
          const hasCriticalAlarm = activeAlarms.some(
            (a) => a.assetId === tagId && a.severity === 'CRITICAL'
          );
    ...
    if (scenario === 'LINE_FAULT') {
      ...
      if (active) {
        ...
        const newAlarm = {
          ...
          description:
            'Cortocircuito en Línea de Transmisión Principal. Disyuntor TRAFO-01 abierto por sobrecorriente.',
          severity: 'CRITICAL',
  ...
  async updateTelemetryStep() {
    ...
    solarAssets.forEach((tag) => {
      ...
      if (breaker && irradiance > 0) {
        powerkW = (irradiance * 8000 * 0.2 * (1 - 0.004 * (panelTemp - 25))) / 1000;
      ...
      if (tag === 'INV-02' && breaker) {
        ...
        if (temp > 82 && !activeAlarms.some((a) => a.alarmCode === 'ALM_INV_OVERTEMP')) {
          ...
          activeAlarms.push({
            ...
            description:
              'Falla de refrigeración en inversor central - temperatura crítica por filtros tapados',
            severity: 'HIGH',
            triggeredAt: now,
            acknowledgedAt: null,
            clearedAt: null,
    ...
    windAssets.forEach((tag) => {
      ...
      const Cp = Math.max(0, 0.4 * Math.cos(pitchRad));
      ...
      if (tag === 'WTG-01') {
        ...
        if (vibration > 5.8 && !activeAlarms.some((a) => a.alarmCode === 'ALM_WTG_VIBRATION')) {
          ...
          activeAlarms.push({
            ...
            description:
              'Espectro de vibraciones elevado en descansos multiplicadora turbina - anomalía rodamiento',
            severity: 'CRITICAL',
            triggeredAt: now,
            acknowledgedAt: null,
            clearedAt: null,
