...
const TriagePanel = ({ activePatients, onAdmitPatient, onTriggerAlert, simClock }) => {
  ...
  const handleSubmitAdmit = (e) => {
    ...
    onTriggerAlert(`Ingreso Urgencias: Paciente ${newAdmitted.name} categorizado ESI ${newAdmitted.esi}`);
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: '1.5rem' }}>
      ...
      <div className="glass-panel">
        <div className="panel-header">
          ...
          <button className="btn-secondary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.75rem' }} onClick={handleRandomPatient}>
            <RefreshCw size={12} /> Carga Rápida Ficticia
          </button>
        </div>

        <form onSubmit={handleSubmitAdmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            ...
            {searchResults.length > 0 && (
              <div style={{
                ...
                {searchResults.map(p => (
                  ...
                  >
                    <div>
                      <strong>{p.name}</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>({p.age} años, {p.gender})</span>
                    </div>
                    <span style={{ fontFamily: 'monospace', color: 'var(--accent-cyan)', fontSize: '0.7rem' }}>RUT: {p.id}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {selectedPatient && (
            <div style={{
              ...
              <div><strong style={{ color: 'var(--accent-cyan)' }}>Ficha Cargada:</strong> {selectedPatient.name}</div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '6px' }}>
              {[1, 2, 3, 4, 5].map(level => {
                const colors = ["--esi-1-resus", "--esi-2-emerg", "--esi-3-urg", "--esi-4-semi", "--esi-5-non"];
                const labels = ["ESI 1", "ESI 2", "ESI 3", "ESI 4", "ESI 5"];
                const titles = ["Resucitación", "Emergencia", "Urgente", "Semi-urgente", "No urgente"];
                ...
                return (
                  <button
                    ...
                    style={{
                      background: isSelected ? `var(${colors[level-1]})` : 'rgba(255,255,255,0.02)',
                      border: '1px solid',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem' }}>
            <div>
              <label style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '2px' }}>Temp (°C)</label>
              <input type="text" className="form-input" value={temp} onChange={e => setTemp(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '2px' }}>Frec. Cardíaca (LPM)</label>
              <input type="text" className="form-input" value={heartRate} onChange={e => setHeartRate(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '2px' }}>Saturación O₂ (%)</label>
              <input type="text" className="form-input" value={oxySat} onChange={e => setOxySat(e.target.value)} />
            </div>
          </div>
          
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
            <div>
              <label style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '2px' }}>P.A. Sistólica (mmHg)</label>
              <input type="text" className="form-input" value={bpSys} onChange={e => setBpSys(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'block', marginBottom: '2px' }}>P.A. Diastólica (mmHg)</label>
              <input type="text" className="form-input" value={bpDia} onChange={e => setBpDia(e.target.value)} />
            </div>
          </div>
                ...
                <div 
                  ...
                  style={{
                    background: deadPatient 
                      ? 'rgba(0,0,0,0.4)' 
                      : (criticalAlert ? 'rgba(239, 68, 68, 0.08)' : 'rgba(255, 255, 255, 0.02)'),
                    border: '1px solid',
                    borderColor: deadPatient 
                      ? '#ef4444' 
                      : (criticalAlert ? 'var(--esi-1-resus)' : 'var(--border-glass)'),
                    borderRadius: '8px',
                    padding: '0.85rem',
                    position: 'relative',
                    transition: 'var(--transition-smooth)',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <strong style={{ fontSize: '0.85rem', color: deadPatient ? 'var(--text-muted)' : 'var(--text-primary)' }}>
                          {patient.name}
                        </strong>
                      ...
                      {/* Vitals display */}
                      <div style={{ display: 'flex', gap: '0.8rem', fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '6px', fontFamily: 'monospace' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.4rem' }}>
                      ...
                      <div style={{ 
                        ...
                        color: deadPatient 
                          ? '#ef4444' 
                          : (criticalAlert ? '#ef4444' : 'var(--text-secondary)')
                      }}>
                        <Clock size={12} />
                        {deadPatient ? (
                          <span>FALLECIDO</span>
                  ...
                  {criticalAlert && (
                    <div style={{
                      ...
                      <ShieldAlert size={12} />
                      ¡Peligro! Excedió tiempo de espera recomendado. Paciente en degradación clínica.
                    </div>
                  )}
