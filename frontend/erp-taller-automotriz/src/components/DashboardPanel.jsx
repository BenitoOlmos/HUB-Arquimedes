...
const DashboardPanel = ({
  ...
  historicalLogs.forEach((log) => {
    ...
    if (match) {
      ...
      if (day >= -120 && day < -90) {
        periods[0].revenue += log.totalPrice;
        periods[0].cost += log.totalCost;
      } else if (day >= -90 && day < -60) {
        periods[1].revenue += log.totalPrice;
        periods[1].cost += log.totalCost;
      } else if (day >= -60 && day < -30) {
        periods[2].revenue += log.totalPrice;
        periods[2].cost += log.totalCost;
      } else if (day >= -30 && day <= 0) {
        periods[3].revenue += log.totalPrice;
        periods[3].cost += log.totalCost;
      }
    }
  });
    ...
    <div className="panel-view">
      ...
      <div className="grid-2col">
        ...
        <div className="section-card">
          <div className="section-header">
            <h3 className="section-title">
              <TrendingUp size={18} color="var(--accent-blue)" /> Tendencia de Ventas (Últimos 120
              Días)
            </h3>
        ...
        >
          <div className="section-header">
            <h3 className="section-title">
              <AlertTriangle size={18} color="var(--status-inspect)" /> Alertas de Abastecimiento
              Crítico
            </h3>
      ...
      <div className="section-card">
        <div className="section-header">
          <h3 className="section-title">
            <BarChart3 size={18} color="var(--accent-indigo)" /> Vista Rápida del Flujo de Trabajo
            Taller
          </h3>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 'bold' }}>
            Estado de las OTs activas
          </span>
        </div>
        ...
        <div className="grid-3col" style={{ gap: '16px' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {activeOTs
                .filter((o) => o.status === 'EsperandoRepuestos')
                .slice(0, 3)
                .map((ot) => (
                  <div
                    key={ot.id}
