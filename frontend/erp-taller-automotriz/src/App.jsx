...
function App() {
  ...
  const [bays, setBays] = useState([
    { id: 'AND-1', name: 'Elevador 2 Postes', specialty: 'Suspensiones y Transmisión', mechanicId: 'MEC-1' },
    { id: 'AND-2', name: 'Alineación y Balanceo', specialty: 'Alineación y Suspensión', mechanicId: 'MEC-4' },
    { id: 'AND-3', name: 'Foso de Fluidos', specialty: 'Lubricación y Frenos', mechanicId: 'MEC-3' },
    { id: 'AND-4', name: 'Bahía Electrónica', specialty: 'Eléctrico y Diagnóstico', mechanicId: 'MEC-2' }
  ]);
  ...
  const toggleScenario = (key) => {
    setTeacherScenarios(prev => {
      const val = !prev[key];
      const name = key === 'supplierStrike' ? 'Huelga de Transportistas' 
                 : key === 'dollarFluctuation' ? 'Alza del Dólar'
                 : key === 'liftOffline' ? 'Elevador de 2 postes fuera de servicio'
  ...
  const injectVipClient = () => {
    ...
    const newVipOT = {
      ...
      description: "VIP: Revisión urgente de motor, emisión de gases e inyección electrónica. Vehículo corporativo.",
      status: 'Presupuesto',
      andenId: null,
      mechanicId: null,
      partsRequired: [
        { sku: 'SKU-1077', name: 'Filtro de aceite sintético (Universal)', qty: 1, cost: 5000, price: 7500, status: 'Pendiente' },
        { sku: 'SKU-1120', name: 'Sensor de oxígeno O2 (Universal)', qty: 1, cost: 35000, price: 52500, status: 'Pendiente' }
      ],
    ...
    setTurnLogs(prev => [`🚀 EVENTO VIP: Se inyectó la orden ${newVipOT.id} para el cliente VIP ${newVipOT.clientName}.`, ...prev]);
    setCurrentTab('orders');
  };

  const approveOTBudget = (otId) => {
    setActiveOTs(prevOTs => prevOTs.map(ot => {
      if (ot.id === otId) {
        ...
        let note = 'Presupuesto aprobado por cliente. Esperando que se compren/entreguen los repuestos.';

        if (allAvailable) {
          // Find if there's a free anden suitable for it
          const matchedBay = bays.find(b => b.specialty.includes(ot.description.includes('Alineación') ? 'Alineación' : ''));
          if (matchedBay && !activeOTs.some(o => o.andenId === matchedBay.id && o.status === 'EnEjecucion')) {
        ...
        setTurnLogs(logs => [`📋 OT APROBADA: Presupuesto de ${ot.id} aprobado. Estado cambiado a [${nextStatus === 'EnEjecucion' ? 'En Proceso' : 'Espera de Repuestos'}].`, ...logs]);
              ...
              setActiveOTs(prevOTs => prevOTs.map(ot => {
                if (ot.id === shipment.associatedOtId) {
                  ...
                  return {
                    ...
                    notes: nextStatus === 'EnEjecucion' ? 'Repuestos recibidos en andén. Mecánico trabajando.' : 'Repuestos recibidos. Esperando asignación de andén.'
                  };
                }
                return ot;
              }));
    ...
    setActiveOTs(prevOTs => {
      return prevOTs.map(ot => {
        if (ot.status === 'EnEjecucion') {
          ...
          if (isSickMec) {
            return { ...ot, notes: '⚠️ Trabajo en pausa: Carlos Mendoza está ausente por licencia médica.' };
          }
          if (isOfflineBay) {
            return { ...ot, notes: '⚠️ Trabajo en pausa: Elevador 2 postes bloqueado por falla de presión.' };
          }
          ...
          return {
            ...
            notes: isDone ? 'Servicio completado. Estacionado en patio listo para entrega.' : `En progreso. Avance: ${Math.round((newHoursReal/ot.laborHoursTarget)*100)}%`
          };
        }
        return ot;
      });
    ...
    <div className="app-container">
      ...
      <header className="app-header">
        <div className="header-brand">
          ...
          <h1 className="brand-title">
            <Wrench size={18} style={{ color: 'var(--accent-blue)' }} /> ERP Simulación Taller Automotriz
          </h1>
        </div>
        ...
        <nav className="global-navbar">
          ...
          <button 
            ...
            style={{ border: '1px dashed var(--status-inspect)', background: currentTab === 'teacher' ? 'rgba(245,158,11,0.1)' : '' }}
          >
            <Sliders size={14} />
      ...
      <footer className="sim-clock-bar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', fontWeight: 'bold' }}>
          ...
          <span style={{ color: 'var(--text-secondary)' }}>Envíos pendientes de entrega: {pendingShipments.length}</span>
        </div>

        {/* Turn logs ticker */}
        <div style={{ flex: 1, margin: '0 24px', fontSize: '0.75rem', color: 'var(--text-secondary)', background: 'rgba(0,0,0,0.03)', padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-glass)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '500px' }}>
        ...
        <button 
          ...
          onClick={advanceTurn}
          style={{ gap: '6px', padding: '8px 20px', background: 'linear-gradient(135deg, var(--accent-blue) 0%, var(--accent-indigo) 100%)', border: 'none', color: '#ffffff' }}
        >
